  			...
  			<el-col :span="12" :offset="4">
				<el-form :model="formData" :rules="rules" ref="formData" label-width="110px" class="demo-formData">
					...
					<el-form-item label="联系电话" prop="phone">
						<el-input type="number" v-model.number="formData.phone" maxLength="11"></el-input>
					</el-form-item>
...
<script>
	...
	export default {
		data(){
			return {
				...
				activities: [{
							...
							icon_name: '减1',
							name: '满减优惠',
							description: '满50减5，满60减8',
					},{
							icon_name: '减2',
							name: '满减优惠',
							description: '满80减5，满60减8',
					}],
					baseUrl,
					baseImgPath,
					categoryOptions: [],
    	...
    	methods: {
    		async initData(){
    			try{
    				let cityData = await cityGuess()
						this.city = cityData.data
						let categoriesData = await foodCategory()
    		...
    		async querySearchAsync(queryString, cb) {
    			if (queryString) {
	    			try{
							let cityListData = await searchplace(this.city.id, queryString);
							let cityList = cityListData.data
			...
			handleShopAvatarScucess(res, file) {
				if (res.status == 200) {
					this.formData.image_path = res.data;
				}else{
			...
			handleBusinessAvatarScucess(res, file) {
				if (res.status == 200) {
					this.formData.business_license_image = res.data;
				}else{
					this.$message.error('上传图片失败！');
				}
			},
			handleServiceAvatarScucess(res, file) {
				if (res.status == 200) {
					this.formData.catering_service_license_image = res.data;
				}else{
					this.$message.error('上传图片失败！');
				}
			},
